...
export default function TextGifCanvas() {
  ...
  useEffect(() => {
    ...
    const LAND_COLOR = "#eeff00ff";  // Bright gold/yellow
    const SEA_COLOR = "#ffffffff";   // White
    const BACKGROUND_COLOR = "#000000ff"; // Dark blue from your code
    ...
    fetch("/earth_big.gif")
      .then(res => {
        if (!res.ok) {
          throw new Error(`Failed to load GIF: ${res.statusText}`);
        }
      ...
      .then(buffer => {
        try {
          ...
          if (!frames[0] || !frames[0].dims) {
            console.error("Invalid frame structure");
            return;
          }
